...
if os.name == "nt":
    try:
        ...
        ctypes.windll.shell32.SetCurrentProcessExplicitAppUserModelID(
            u"skyfors.SubWords"  # любое стабильное имя приложения
        )
    except Exception:
        pass
...
def bin_path(exe_name: str) -> str:
    p = resource_path(exe_name)
    return p if os.path.exists(p) else exe_name  # если нет рядом с exe – берём из PATH
...
IRREGULAR_FILE = pathlib.Path("irregular_keep.txt")
NAMES_FILE     = pathlib.Path("names_exclude.txt")

def ensure_from_bundle(src_name: str, dst_path: pathlib.Path):
    """Если файла нет рядом с exe — копируем его из ресурсов PyInstaller."""
    try:
        if not dst_path.exists():
            bundled = resource_path(src_name)   # путь внутри бандла (строка)
            if os.path.exists(bundled):
                shutil.copyfile(bundled, str(dst_path))
    except Exception:
        pass

ensure_from_bundle("irregular_keep.txt", IRREGULAR_FILE)
ensure_from_bundle("names_exclude.txt",  NAMES_FILE)

def load_names_file(p: pathlib.Path) -> set:
    try:
        if p.exists():
...
NAMES_EXCLUDE = load_names_file(NAMES_FILE)

def load_exceptions_file(p: pathlib.Path) -> set:
    """По желанию пользователь может добавлять свои формы построчно в irregular_keep.txt"""
    try:
        if p.exists():
            lines = p.read_text(encoding="utf-8", errors="ignore").splitlines()
            return {re.sub(r"[^A-Za-z']", "", ln.strip()).lower()
                    for ln in lines if ln.strip()}
    except Exception:
        pass
    return set()

IRREGULAR_KEEP = load_exceptions_file(IRREGULAR_FILE)

# ---------- Helpers ----------
def run(cmd):
    # Глушим консольные окна у дочерних процессов на Windows
    if os.name == "nt":
...
def lemmatize(words):
    """
    Лемматизируем, но формы из IRREGULAR_KEEP оставляем как есть.
    Имена из NAMES_EXCLUDE полностью исключаем.
    """
    if USE_SPACY and NLP is not None:
        ...
        for t in doc:
            if not t.is_alpha:
                continue
            w = t.text.lower()
            if w in NAMES_EXCLUDE:
                continue                 # имя: исключаем
            if w in IRREGULAR_KEEP:
                out.append(w)            # неправильная форма: не лемматизируем
            else:
                out.append(t.lemma_.lower())
        return out
    # fallback без spaCy: тоже исключаем имена
    return [_simple_en_lemma(w) for w in words if w.lower() not in NAMES_EXCLUDE]



def _simple_en_lemma(w: str) -> str:
    s = w.strip().lower()
    # Убираем possessive
    if s.endswith(("'s", "’s")) and s.count("'") <= 1 and s.count("’") <= 1:
        s = s[:-2]
    ...
    if len(s) >= 3:
        if s.endswith("ies") and len(s) > 3:
            return s[:-3] + "y"
        for suf in ("sses", "xes", "ches", "shes", "zes"):
            if s.endswith(suf) and len(s) > len(suf):
                return s[:-2]  # classes→class, boxes→box
...
class App(tk.Tk):
    ...
    def _refresh_counters(self):
        # Пересчёт на основе уникальных ЛЕММ из последнего анализа
        lemmas_set = getattr(self, "_lemmas_set", None) or getattr(self, "_all_unique_set", None)
        if lemmas_set is None:
            return
        total = len(lemmas_set)
        unknown_unique = len(lemmas_set - self.known_words)
        studying_found = len(lemmas_set & self.studying_words)
    ...
    def _analyze_worker(self, ipath: str):
        try:
            ...
            lemmas = [w for w in lemmas if w not in STOPLIST and w not in NAMES_EXCLUDE and len(w) > 1]
...
# База = уникальные токены после того же фильтра, что и для таблицы:
                        # --- СЧЁТЧИКИ И ЧАСТОТЫ ПО ЛЕММАМ ---
            # База для счётчиков — уникальные ЛЕММЫ после фильтра стоп-слов
            lemmas_base = set(lemmas)
            total_tokens = len(lemmas_base)

            # неизвестные считаем по леммам, а не по исходным токенам
            unknown_lemmas = [w for w in lemmas if w not in known]
            freq = Counter(unknown_lemmas)
            unknown_unique = len(freq)

            studying_found = len(lemmas_base & self.studying_words)

            # отдадим состояние для мгновенного пересчёта счётчиков в GUI
            self._q.put({
                'kind': 'state',
                'lemmas_set': list(lemmas_base),
                'all_unique': list(lemmas_base),  # _refresh_counters использует это поле
            })
